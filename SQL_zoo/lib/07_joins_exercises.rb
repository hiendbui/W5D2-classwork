...
def travoltas_busiest_years
  ...
  #Show the year and the
  # number of movies he made for any year in which he 
  #made at least 2 movies.
  execute(<<-SQL)
    SELECT
      yr, COUNT(movies) AS num_movies
    FROM
      movies
    JOIN
      castings ON movies.id = castings.movie_id
...
def films_by_cast_size
  ...
  # the number of actors in the cast (descending), then 
  # by title (ascending).
  execute(<<-SQL)
...
def colleagues_of_garfunkel
  # List all the people who have played 
  # alongside 'Art Garfunkel'.
  execute(<<-SQL)
    SELECT
      DISTINCT name
    FROM
